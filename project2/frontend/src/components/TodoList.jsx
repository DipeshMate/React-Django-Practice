    ...
    <div className="todo-list">
        ...
        <div className="todo-list-item" key={ index }>
             <div className="task">
              ...
              <p className={todo.status === "Completed" ? "strike" : ""}>{todo.task}</p>
              <small>{todo.description}</small>
            </div>
            <div className="btn-container">
              <div className="edit">
                <TbEdit size={25} onClick={() => todoItem(todo.task, todo.description, todo.id)} />
              </div>
              <div className="del">
    ...
    { toggle &&  <div className="modal-container">
      <div className="modal">
        <h1>Update Todo</h1>
          ...
          <form action="" onSubmit = { (e) => {update_todo(e, todoId, task, desc); setToggle(false)} }>
            <input type="text" ref={ taskRef } placeholder="Update Todo" value={task} onChange={(e)=>setTask(e.target.value)} required />
            <input type="text" placeholder="Update Description" value={desc} onChange={(e) => setDesc(e.target.value)} required />
